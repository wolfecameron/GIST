            ...
            if self.args.rank == 0:
                for layer_idx in range(len(all_indices) + 1):
                    if layer_idx == 0:
                        ...
                        self.sub_model.layers[layer_idx].linear.weight.data = correct_cols[next_idx, :]
                        self.sub_model.layers[layer_idx].linear.bias.data = (
                            self.base_model.layers[layer_idx].linear.bias.data[next_idx])
                    ...
                    for site_i in range(1, self.args.num_subnet):
                        ...
                        if self.args.rank == 0:
                            ...
                            broadcast_weight(
                                ...
                                source=0)
                            broadcast_weight(
                                self.base_model.layers[layer_idx].linear.bias.data[next_tens],
                                rank_list=[0, site_i], source=0)
                        else:
                            broadcast_weight(
                                self.sub_model.layers[layer_idx].linear.weight.data, rank_list=[0, site_i],
            ...
            if (
                ...
                if (
                    ...
                    if args.rank == 0:
                        ...
                        if args.use_f1:
                            val_acc = evaluate(ist_model.base_model, g, labels, val_mask, 'f1')
                            test_acc = evaluate(ist_model.base_model, g, labels, test_mask, 'f1')
                        else:
                            val_acc = evaluate(ist_model.base_model, g, labels, val_mask)
                            test_acc = evaluate(ist_model.base_model, g, labels, test_mask)
