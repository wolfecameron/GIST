# python script/amazon/run_ist_sweep_amazon_ultrawide.py
...
num_partition = 15000
TEST_NAME = f'amazon_ultrawide_ps{num_partition}'
                        ...
                        for i in range(num_trials):
                            ...
                            if num_ist == 1:
                                command = (
                                    ...
                                    f' --dropout {dropout} --normalize --rnd-seed {i} --use-f1 --eval-cpu'
                                ...
                                for ist_id in range(num_ist):
                                    sub_command = (
                                        f'srun -p dsta --mpi=pmi2 --gres=gpu:1 -n1 --ntasks-per-node=1 --job-name={TEST_NAME} '
                                        f'--kill-on-bad-exit=1 -w SG-IDC1-10-51-2-73 '
                                        f'python cluster_gcn/cluster_gcn_ist_ultra_wide.py'
